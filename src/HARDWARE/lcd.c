...
{  							  
	...
	for(t=0;t<72;t++)
	{   
		if(size==24) temp=CH_matrix_24[num][t];
		else if(size==16) temp=CH_matrix_16[num][t];
		else break;
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,POINT_COLOR);
...
//x,y:起点坐标
//*cn 中文字符指针
void LCD_ShowFontHZ(u16 x,u16 y,u16 width,u16 height,u8 size)
{
	u8 a=0;
	u8 x0=x;
	width+=x;
